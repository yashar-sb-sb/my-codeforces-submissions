...
bool f()
{
	...
	r0 = i;
	r1 = j;
	if((i>Cs[0].size()?(LL)1e18:Mi[0]*Cs[0][i-1]) + (j>Cs[1].size()?(LL)1e18:Mi[1]*Cs[1][j-1]) <= s)return true;
	st = 1;
		if((i>C[0].size()?(LL)1e18:Mi[0]*C[0][i-1].first) > (j>C[1].size()?(LL)1e18:Mi[1]*C[1][j-1].first))
		{
			i -= st;
	...
	r0 = i;
	r1 = j;
	return (i>Cs[0].size()?(LL)1e18:Mi[0]*Cs[0][i-1]) + (j>Cs[1].size()?(LL)1e18:Mi[1]*Cs[1][j-1]) <= s;
}

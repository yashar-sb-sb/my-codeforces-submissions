...
int main()
{
	...
	for(int i = 0; i < n; ++i)
	{
		...
		for(auto k:cur)
		{
			if(k.dx && k.dy)
			{
				nex.push_back(tok(k.x+k.dx, k.y, k.dx, 0));
				nex.push_back(tok(k.x, k.y+k.dy, 0, k.dy));
			}
			else if(k.dx)
			{
				nex.push_back(tok(k.x+k.dx, k.y-1, k.dx, -1));
				nex.push_back(tok(k.x+k.dx, k.y+1, k.dx, 1));
			}
			else if(k.dy)
			{
				nex.push_back(tok(k.x-1, k.y+k.dy, -1, k.dy));
				nex.push_back(tok(k.x+1, k.y+k.dy, 1, k.dy));
			}
		}
	}

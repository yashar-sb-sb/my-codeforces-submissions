...
ios_base::sync_with_stdio(0);
	...
	for(int i = 0; i < n; ++i)
	{
		...
		if(min(m[pii(A[i][0],A[i][1])].first+A[i][2],min(A[i][0],A[i][1])) > ma)
		{
			res = i+1;
			res1 = m[pii(A[i][0],A[i][1])].second;
			ma = min(m[pii(A[i][0],A[i][1])].first+A[i][2],min(A[i][0],A[i][1]));
		}
		if(min(m[pii(A[i][0],A[i][2])].first+A[i][1],min(A[i][0],A[i][2])) > ma)
		{
			res = i+1;
			res1 = m[pii(A[i][0],A[i][2])].second;
			ma = min(m[pii(A[i][0],A[i][2])].first+A[i][1],min(A[i][0],A[i][2]));
		}
		if(min(m[pii(A[i][1],A[i][2])].first+A[i][0],min(A[i][1],A[i][2])) > ma)
		{
			res = i+1;

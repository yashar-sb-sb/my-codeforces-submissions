...
bool f(pii i, pii j)
{
	...
	if(a < b)return true;
	if(b < a)return false;
	return i.first * j.second - j.first * i.second > 0;
}
...
ios_base::sync_with_stdio(0);
	...
	sort(A+0,A+n,f);
	ldb res = -numeric_limits<ldb>::infinity(), temp,temp1;
	for(int i = 0; i < n; ++i)
	{
		temp = (A[i].first*A[(i+1)%n].first+A[i].second*A[(i+1)%n].second);
		temp1 = (ldb)sqrt(A[i].first*A[i].first+A[i].second*A[i].second)*sqrt(A[(i+1)%n].first*A[(i+1)%n].first+A[(i+1)%n].second*A[(i+1)%n].second);
		temp /= temp1;

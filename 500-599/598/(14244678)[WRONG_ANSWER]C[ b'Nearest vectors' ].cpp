...
ios_base::sync_with_stdio(0);
	...
	for(int i = 0; i < n; ++i)
	{
		temp = acos((ldb)(A[i].first*A[(i+1)%n].first+A[i].second*A[(i+1)%n].second)/sqrt(A[i].first*A[i].first+A[i].second*A[i].second)/sqrt(A[(i+1)%n].first*A[(i+1)%n].first+A[(i+1)%n].second*A[(i+1)%n].second));
